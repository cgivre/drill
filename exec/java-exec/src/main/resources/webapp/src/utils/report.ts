 ...
 * model and when.
 *
 * The model and provider are deliberately not named. They live only on the
 * admin-only AiConfig, and mirroring them onto /api/v1/ai/status so the browser
 * could read them would disclose the deployment's LLM to every authenticated
 * user. See docs/dev/ProspectorReports.md.
 */
export function provenanceFooter(p: ReportProvenance): string {
  ...
  return `\n\n---\n\n*Generated by Prospector on ${when}.${source} `
